...
<div class="d-flex w-100 justify-content-center align-items-center mb-3 __min-h-70px __inline-35"
    style="background-image:url(https://cdn.pixabay.com/photo/2016/07/08/13/37/texture-1504364_1280.jpg);height:200px;">

    <div class="text-capitalize container">
        <span class="__text-18px font-semibold">{{\App\CPU\translate(str_replace('_',' ',$data['data_from']))}}
            {{\App\CPU\translate('products')}} {{ isset($brand_name) ? '('.$brand_name.')' : ''}} </span>
    </div>

</div>
<!-- Page Content-->
<div class="container pb-5 mb-2 mb-md-4 rtl __inline-35" style="text-align: {{Session::get('direction') === " rtl"
    ? 'right' : 'left' }};">
    <div class="row">
        <!-- Sidebar-->
        <aside
                            ...
                            <label class="w-100 opacity-75 text-nowrap for-shoting d-block mb-0" for="sorting"
                                ...
                                <select class="form-control custom-select" id="searchByFilterValue">
                                    ...
                                    <option
                                        ...
                                        {{isset($data['data_from'])!=null?$data['data_from']=='best-selling'
                                        ?'selected':'':''}}>{{\App\CPU\translate('best_selling_product')}}</option>
                                    <option
                                        value="{{route('products',['id'=> $data['id'],'data_from'=>'top-rated','page'=>1])}}"
                                        {{isset($data['data_from'])!=null?$data['data_from']=='top-rated'
                                        ?'selected':'':''}}>{{\App\CPU\translate('top_rated')}}</option>
                                    <option
                                        value="{{route('products',['id'=> $data['id'],'data_from'=>'most-favorite','page'=>1])}}"
                                        {{isset($data['data_from'])!=null?$data['data_from']=='most-favorite'
                                        ?'selected':'':''}}>{{\App\CPU\translate('most_favorite')}}</option>
                                    <option
                                        value="{{route('products',['id'=> $data['id'],'data_from'=>'featured_deal','page'=>1])}}"
                                        {{isset($data['data_from'])!=null?$data['data_from']=='featured_deal'
                                        ?'selected':'':''}}>{{\App\CPU\translate('featured_deal')}}</option>
                                </select>
                            </label>
                        </div>
                    </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center __cate-side-price">
                            ...
                            <div class="d-flex justify-content-center align-items-center __number-filter-btn">
                                ...
                                <a class="" onclick="searchByPrice()">
                                    <i class="__inline-37 czi-arrow-{{Session::get('direction') === " rtl" ? 'left'
                                        : 'right' }}"></i>
                                </a>
                        ...
                        <ul id="lista1" class="__brands-cate-wrap" data-simplebar data-simplebar-auto-hide="false">
                            ...
                            <div class="brand mt-2 for-brand-hover {{Session::get('direction') === " rtl" ? 'mr-2' : ''
                                ...
                                <li class="flex-between __inline-39" 
                                    onclick1="location.href='{{route('products',['id'=> $brand['id'],'data_from'=>'brand','page'=>1])}}'"
                                    onclick="redirectToProducts('{{route('products',['id'=> $brand['id'],'data_from'=>'brand','page'=>1])}}')"
                                >
                                    <div>
                                        {{ $brand['name'] }}
                                    </div>
                                    ...
                                    @endif
                                    <input type="checkbox" id="vehicle1" name="vehicle1" value="{{$brand['id']}}"
                                ...
                                <div class="card-header p-1 flex-between">
                                    <div>
                                        <label class="for-hover-lable cursor-pointer"
                                            onclick1="location.href='{{route('products',['id'=> $category['id'],'data_from'=>'category','page'=>1])}}'"
                                            onclick="redirectToProducts('{{ route('products',['id'=> $category['id'],'data_from'=>'category','page'=>1]) }}')">
                                            {{$category['name']}}
                                        </label>
                                    ...
                                    <div class=" for-hover-lable card-header p-1 flex-between">
                                        <div>
                                            <label class="cursor-pointer"
                                                onclick1="location.href='{{route('products',['id'=> $child['id'],'data_from'=>'category','page'=>1])}}'"
                                                onclick="redirectToProducts('{{ route('products',['id'=> $child['id'],'data_from'=>'category','page'=>1]) }}')">
                                                {{$child['name']}}
                                            </label>
                                        </div>
                                    ...
                                    <div class="card-body p-0 {{Session::get('direction') === " rtl" ? 'mr-2' : 'ml-2'
                                        ...
                                        <div class="card-header p-1">
                                            <label class="for-hover-lable d-block cursor-pointer text-left"
                                                onclick1="location.href='{{route('products',['id'=> $ch['id'],'data_from'=>'category','page'=>1])}}'"
                                                onclick="redirectToProducts('{{ route('products',['id'=> $ch['id'],'data_from'=>'category','page'=>1]) }}')">{{$ch['name']}}</label>
                                        </div>
        ...
        <section class="col-lg-9">
            <div class="d-flex flex-wrap align-items-center justify-content-between __inline-43 __gap-6px p-2">
                ...
                <h1 class="max-sm-order-1">
                    <label id="price-filter-count"> {{$products->total()}} {{\App\CPU\translate('items found')}}
                    </label>
                </h1>
                        ...
                        <form id="search-form" action="{{ route('products') }}" method="GET">
                            ...
                            <div class=" {{Session::get('direction') === " rtl" ? 'float-left' : 'float-right' }}">
                                ...
                                <select class="__inline-44" onchange="filter(this.value)">
                                    ...
                                    <option value="z-a">{{\App\CPU\translate('Z_to_A')}} {{\App\CPU\translate('Order')}}
                                    </option>
                                </select>
                            </div>
                        </form>
...
<script>
    ...
    function filter(value) {
        $.get({
            ...
            data: {
                ...
                sort_by: value,
                brand_id:localStorage.getItem('selectedBrand'),
            },
            dataType: 'json',
            beforeSend: function () {
                $('#loading').show();
    ...
    // Define a variable to store the selected category and brand IDs
    ...
    function updateSelectedCategory(categoryId) {
        ...
        // Update your frontend UI to reflect the selected category state (e.g., check the corresponding checkbox)
        // Make sure to call this function when a category is clicked.
    }

    // Function to update the selected brand
    function updateSelectedBrand(brandId) {
        selectedBrand = brandId;
        console.log(selectedBrand)
        const brandCheckboxes = document.querySelectorAll('.brand-checkbox');

        // Deselect all other brand checkboxes
        ...
        // Check if any brand checkbox is checked, and update the local storage value accordingly
        const anyBrandChecked = Array.from(brandCheckboxes).some((checkbox) => checkbox.checked);
        if (anyBrandChecked) {
            // If at least one brand checkbox is checked, store the selectedBrand value in local storage
            localStorage.setItem('selectedBrand', brandId);
        } else {
            // If no brand checkboxes are checked, store a blank value (empty string) in local storage
            localStorage.setItem('selectedBrand', '');
        }
    }

    // When a category or brand is clicked, call the corresponding function to update the state.
    // For example, assuming you have a list of categories and brands:
    ...
    document.addEventListener('DOMContentLoaded', () => {
...

        {
                ...
                {{-- if (storedCategory) {
                    // Set the selected category checkbox based on the stored value
                    const categoryCheckbox = document.querySelector(`input[value="${storedCategory}"]`);
                    if (categoryCheckbox) {
                        categoryCheckbox.checked = true;
                        updateSelectedCategory(storedCategory);
                    }
                --}}
        }

        if (storedBrand) {
            // Set the selected brand checkbox based on the stored value
            const brandCheckbox = document.querySelector(`input[value="${storedBrand}"]`);
            if (brandCheckbox) {
                brandCheckbox.checked = true;
                updateSelectedBrand(storedBrand);
            }
        }
    ...
    function redirectToProducts(url) {
        const brandId = localStorage.getItem("selectedBrand");
        // If brandId is null or empty, you may want to handle it accordingly or skip appending the parameter.
        // Otherwise, you can include the brand_id parameter in the URL.
        if (brandId) {
